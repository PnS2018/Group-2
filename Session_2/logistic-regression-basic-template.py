...
for z in range(num_epochs):
    combined_data = np.concatenate((train_X, np.expand_dims(train_y, axis=1)), axis=1)
    shuffle(combined_data)
    current_train_x= combined_data[:, :-1]
    current_train_y= combined_data[:, -1:]
    
    for i in range(num_train_samples//batch_size):
        train_function((current_train_x[i*batch_size:(i+1)*batch_size],current_train_y[i*batch_size:(i+1)*batch_size]))
        
    for i in range(num_train_samples//batch_size):
        test_function((current_train_x[i*batch_size:(i+1)*batch_size],current_train_y[i*batch_size:(i+1)*batch_size]))
